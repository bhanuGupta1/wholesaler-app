...
export const FallbackQRGenerator = ({ 
  ...
  const generateQRCode = async () => {
    ...
    try {
      ...
      const encodedData = encodeURIComponent(typeof data === 'object' ? JSON.stringify(data) : data);
...
export const FallbackQRScanner = ({ 
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      setError('QR scanning requires camera libraries. Please install html5-qrcode for full functionality.');
      ...
      <div className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl shadow-lg border overflow-hidden`}>
        ...
        <div className="p-6">
          ...
          <div className={`mt-4 p-4 rounded-lg ${darkMode ? 'bg-yellow-900/20 border-yellow-800' : 'bg-yellow-50 border-yellow-200'} border`}>
            <h4 className={`font-medium ${darkMode ? 'text-yellow-300' : 'text-yellow-800'} mb-2`}>
              🔧 Enable Full QR Functionality
            </h4>
            <p className={`text-sm ${darkMode ? 'text-yellow-200' : 'text-yellow-700'} mb-2`}>
              To enable camera scanning and advanced features, install the required libraries:
            </p>
            <code className={`text-xs ${darkMode ? 'bg-gray-700 text-gray-300' : 'bg-white text-gray-800'} p-2 rounded block`}>
