...
import { parseQRData, handleQRNavigation, downloadQRCode } from '../../utils/qrUtils';
...
const QRCodeViewer = ({ 
  ...
  return (
    <div className={`${className} text-center`}>
      ...
      {/* QR Data Preview */}
      <div className={`mt-3 p-2 rounded text-xs ${darkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-600'}`}>
        <details>
          <summary className="cursor-pointer">
            {parsed.isOurFormat ? `${parsed.type}: ${parsed.data.name || parsed.data.customer || parsed.data.email || parsed.id}` : 'QR Data'}
          </summary>
          <pre className="mt-2 text-left break-all whitespace-pre-wrap max-h-20 overflow-y-auto">
            {qrString.length > 200 ? qrString.substring(0, 200) + '...' : qrString}
          </pre>
        </details>
      </div>
    </div>
