...
const CompleteQRScanner = ({ 
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      if (librariesLoaded.zxing) {
        ...
        const result = await codeReader.decodeFromInputVideoDevice(undefined, 'qr-reader-file');
      ...
      setError('Failed to scan QR code from file. Please try a different image or install the required libraries.');
    }
  ...
  const scanFileOnline = async (file) => {
    ...
    setError('File scanning requires QR libraries to be installed. Please install html5-qrcode and @zxing/library packages.');
  };
    ...
    <div className={`${className} max-w-2xl mx-auto`}>
      <div className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl shadow-lg border overflow-hidden`}>
        ...
        <div className={`px-6 py-4 border-b ${darkMode ? 'border-gray-700 bg-gray-700' : 'border-gray-200 bg-gray-50'}`}>
          ...
          {/* Camera Selection */}
          {scanMethod === 'camera' && cameras.length > 1 && librariesLoaded.html5qrcode && (
            <div className="mt-3">
        ...
        <div className="p-6">
          ...
          {libraryStatus.percentage < 100 && (
            <div className={`mb-4 p-3 rounded-lg ${darkMode ? 'bg-yellow-900/20 border-yellow-800' : 'bg-yellow-50 border-yellow-200'} border`}>
              <div className="flex items-start">
                <div className="text-2xl mr-3">⚠️</div>
                <div>
                  <h4 className={`font-medium ${darkMode ? 'text-yellow-400' : 'text-yellow-800'} mb-1`}>
                    Limited QR Functionality
                  </h4>
                  <p className={`text-sm ${darkMode ? 'text-yellow-300' : 'text-yellow-700'} mb-2`}>
                    Some QR libraries are missing. Install them for full functionality:
                  </p>
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div className={`flex items-center ${librariesLoaded.qrcode ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                      {librariesLoaded.qrcode ? '✅' : '❌'} qrcode
                    </div>
                    <div className={`flex items-center ${librariesLoaded.html5qrcode ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                      {librariesLoaded.html5qrcode ? '✅' : '❌'} html5-qrcode
                    </div>
                    <div className={`flex items-center ${librariesLoaded.zxing ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
          ...
          {scanResult && (
            <div className={`mb-4 p-3 rounded-lg ${darkMode ? 'bg-green-900/20 border-green-800' : 'bg-green-50 border-green-200'} border`}>
              ...
              <div className={`p-2 rounded ${darkMode ? 'bg-gray-700' : 'bg-white'}`}>
                <pre className={`text-xs ${darkMode ? 'text-gray-300' : 'text-gray-600'} break-all whitespace-pre-wrap max-h-32 overflow-y-auto`}>
            ...
            <div className="text-center">
              {librariesLoaded.html5qrcode ? (
                ...
                <div className={`p-8 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                  ...
                  <code className={`text-xs ${darkMode ? 'bg-gray-800 text-gray-300' : 'bg-white text-gray-800'} p-2 rounded block`}>
          ...
          {libraryStatus.percentage < 100 && (
            <div className={`mt-4 p-4 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
              ...
              <code className={`text-xs ${darkMode ? 'bg-gray-800 text-gray-300' : 'bg-white text-gray-800'} p-2 rounded block font-mono`}>
