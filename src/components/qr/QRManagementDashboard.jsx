...
const QRManagementDashboard = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Quick Actions */}
      <div
        className={`${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"} rounded-xl shadow-lg border p-6`}
      >
        <h2
          className={`text-xl font-bold ${darkMode ? "text-white" : "text-gray-800"} mb-4`}
        >
          ⚡ Quick Actions
        </h2>
      ...
      {/* Main Content */}
      <div
        className={`${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"} rounded-xl shadow-lg border overflow-hidden`}
      >
        {/* Tab Header */}
        <div
            ...
            <div className="text-center py-12">
              ...
              <p className={`${darkMode ? "text-gray-400" : "text-gray-500"}`}>
                {searchTerm
                  ...
                  : `No ${activeTab} available for QR generation`}
              </p>
            </div>
          )}
        </div>
...
const ItemCard = ({ item, type, isSelected, onSelect, darkMode }) => {
  ...
  const getItemTitle = () => {
    switch (type) {
      case "products":
        return item.name;
      case "orders":
        return `Order #${item.id.slice(0, 8)}`;
      case "users":
  ...
  const getItemSubtitle = () => {
    switch (type) {
      case "products":
        return `SKU: ${item.sku} • $${item.price}`;
      case "orders":
        return `${item.customerName} • $${item.total}`;
      case "users":
        return item.email;
      default:
        return "";
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {qrData.map(({ item, qrCodeDataUrl, filename }, index) => (
              ...
              >
                ...
                />
                <h4
                  className={`font-medium ${darkMode ? "text-white" : "text-gray-900"} mb-1`}
                >
                  {item.name ||
                    item.customerName ||
                    item.email ||
...
const ScannerModal = ({ isOpen, onClose, onScan, scanResult, darkMode }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div
        className={`${darkMode ? "bg-gray-800" : "bg-white"} rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden`}
      >
        <div
          className={`px-6 py-4 border-b ${darkMode ? "border-gray-700" : "border-gray-200"} flex justify-between items-center`}
        >
        ...
        <div className="p-6">
          ...
          {scanResult && (
            ...
            >
              <h4
                className={`font-medium ${darkMode ? "text-green-400" : "text-green-700"} mb-2`}
              >
                ✅ Scan Result:
              </h4>
              <pre
                className={`text-sm ${darkMode ? "text-gray-300" : "text-gray-600"} break-all whitespace-pre-wrap`}
              >
