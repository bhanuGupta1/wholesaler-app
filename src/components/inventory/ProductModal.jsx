...
const ProductModal = ({ product, onClose, categories, darkMode }) => {
  ...
  const validate = () => {
    ...
    if (
      formData.costPrice &&
      (isNaN(formData.costPrice) || Number(formData.costPrice) <= 0)
    ) {
      newErrors.costPrice = "Cost price must be a positive number";
    }
    if (
      formData.reorderPoint &&
      (isNaN(formData.reorderPoint) ||
        Number(formData.reorderPoint) < 0 ||
        !Number.isInteger(Number(formData.reorderPoint)))
    ) {
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
      <div
        className={`relative ${darkMode ? "bg-gray-800 text-gray-200" : "bg-white text-gray-900"} rounded-lg shadow-xl mx-auto max-w-2xl w-full`}
      >
        {/* Header */}
        <div
          className={`${darkMode ? "bg-gray-700 border-gray-600" : "bg-gray-50 border-gray-200"} py-4 px-6 rounded-t-lg border-b`}
        >
          <div className="flex items-center justify-between">
            <h3
              className={`text-lg font-medium ${darkMode ? "text-gray-200" : "text-gray-900"}`}
            >
        ...
        <form onSubmit={handleSubmit} className="p-6">
          {errors.submit && (
            <div
              className={`mb-4 ${darkMode ? "bg-red-900/30 border-red-800" : "bg-red-50 border-red-400"} border-l-4 p-4 rounded-md`}
            >
              <div className="flex">
                <div className="flex-shrink-0">
                  <svg
                    className={`h-5 w-5 ${darkMode ? "text-red-400" : "text-red-400"}`}
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 20 20"
                    fill="currentColor"
                  >
              ...
              <div>
                ...
                <div className="mt-1 relative rounded-md shadow-sm">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <span
                      className={`${darkMode ? "text-gray-400" : "text-gray-500"} sm:text-sm`}
                    >
                      $
                    </span>
                  </div>
