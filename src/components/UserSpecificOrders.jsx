...
const UserSpecificOrders = () => {
    ...
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        const order = {
          id: doc.id,
          orderId:
            data.orderId || `#ORD-${doc.id.substring(0, 8).toUpperCase()}`,
          customerName:
            data.customerName || data.customer?.name || "Unknown Customer",
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      setOrders((prev) =>
        prev.filter((order) => !selectedOrders.has(order.id)),
      );
      setSelectedOrders(new Set());
      setNotification({
  ...
  if (error) {
    return (
      ...
      >
        <div
          className={`p-6 rounded-lg ${darkMode ? "bg-red-900/20 border-red-800" : "bg-red-50 border-red-200"} border`}
        >
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 py-6 max-w-7xl">
        ...
        <div
          className={`${darkMode ? "bg-gray-800" : "bg-white"} rounded-lg shadow-sm p-6 mb-6`}
        >
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              {/* View Mode Toggle */}
              <div
                className={`flex border rounded-lg ${darkMode ? "border-gray-600" : "border-gray-300"}`}
              >
                <button
              ...
              <div className="ml-auto text-sm text-gray-500">
                {filteredAndSortedOrders.length} of {orders.length} order
                {orders.length !== 1 ? "s" : ""}
              </div>
            </div>
        ...
        {selectedOrders.size > 0 && canDelete && (
          <div
            className={`${darkMode ? "bg-gray-800" : "bg-white"} rounded-lg shadow-sm p-4 mb-6 border border-indigo-200`}
          >
        ...
        {filteredAndSortedOrders.length === 0 ? (
          <div
            className={`${darkMode ? "bg-gray-800" : "bg-white"} rounded-lg shadow-sm p-12 text-center`}
          >
                  ...
                  {paginatedOrders.map((order) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div
                            className={`h-8 w-8 rounded-full ${darkMode ? "bg-gray-600 text-gray-300" : "bg-gray-200 text-gray-600"} flex items-center justify-center font-medium text-sm`}
                          >
        ...
        {totalPages > 1 && (
          <div
            className={`${darkMode ? "bg-gray-800" : "bg-white"} rounded-lg shadow-sm p-4 mt-6`}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <span
