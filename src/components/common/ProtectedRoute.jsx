...
const ProtectedRoute = ({ 
    ...
    return (
      <div className={`flex justify-center items-center h-screen ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
        <div className="text-center">
          ...
          <p className={`text-lg ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Loading...</p>
        </div>
      </div>
    );
  }
  ...
  if (!user.canAccess && approvalStatus && !approvalStatus.canAccess) {
    return <ApprovalStatusScreen approvalStatus={approvalStatus} user={user} darkMode={darkMode} />;
  }
    ...
    switch (approvalStatus.status) {
      case 'pending_approval':
        return {
          ...
          message: 'Your account is currently being reviewed by our admin team.',
          color: 'yellow',
          details: [
      ...
      default:
        return {
          ...
          message: 'Your account requires approval before you can access this area.',
          color: 'blue',
          details: [
      ...
      <div className={`max-w-md w-full p-8 ${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg text-center`}>
        ...
        <div className={`p-4 rounded-lg mb-6 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
          <div className="text-left space-y-2">
            ...
            {user.businessName && (
              <div className="flex justify-between">
                <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Business:</span>
                <span className={`text-sm font-medium ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>
                  {user.businessName}
                </span>
              </div>
...
const AccessDeniedScreen = ({ reason, userRole, darkMode, missingPermission }) => {
  return (
    <div className={`min-h-screen flex items-center justify-center ${darkMode ? 'bg-gray-900' : 'bg-gray-50'} px-4`}>
      <div className={`max-w-md w-full p-8 ${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg text-center`}>
        ...
        <div className={`p-4 rounded-lg mb-6 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
          <div className="text-left space-y-2">
            <div className="flex justify-between">
              <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Your Role:</span>
              <span className={`text-sm font-medium ${darkMode ? 'text-gray-200' : 'text-gray-800'} capitalize`}>
                {userRole}
              </span>
            </div>
            {missingPermission && (
              <div className="flex justify-between">
                <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Required:</span>
