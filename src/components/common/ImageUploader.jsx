  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      <div className="mt-1 flex items-center space-x-4">
        {/* Image Preview */}
        {showPreview && (
          <div className={`flex-shrink-0 ${previewSizes[previewSize]} ${darkMode ? 'bg-gray-700' : 'bg-gray-100'} rounded-md overflow-hidden flex items-center justify-center relative`}>
            {imagePreview ? (
        ...
        <div className="flex flex-col flex-1">
          ...
          {isUploading && uploadProgress > 0 && (
            <div className="mt-2">
              <div className="flex justify-between text-xs mb-1">
                <span className={darkMode ? 'text-gray-400' : 'text-gray-500'}>Uploading...</span>
                <span className={darkMode ? 'text-gray-400' : 'text-gray-500'}>{uploadProgress}%</span>
              </div>
              <div className={`w-full h-1.5 bg-gray-200 rounded-full overflow-hidden ${darkMode ? 'bg-gray-700' : 'bg-gray-200'}`}>
                <div 
          ...
          <p className={`text-xs mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            Recommended: Square image (e.g., 600×600px), JPEG or PNG, under {maxSizeMB}MB
          </p>
        </div>
      </div>
    </div>
