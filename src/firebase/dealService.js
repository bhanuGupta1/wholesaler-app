...
export class DealService {
  ...
  async createDeal(dealData, userId) {
    try {
      ...
      const deal = {
        ...
        minPurchase: dealData.minPurchase
          ? parseFloat(dealData.minPurchase)
          : 0,
  ...
  async applyDealToOrder(dealId, orderId, orderAmount, userId) {
    try {
      ...
      // Log the activity
      await this.logDealActivity(dealId, "applied", userId, {
        orderId,
        orderAmount,
      });
  ...
  async getDealAnalytics(dealId) {
    try {
      ...
      // Calculate totals
      const totalDiscount = usageRecords.reduce(
