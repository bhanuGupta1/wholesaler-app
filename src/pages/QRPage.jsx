...
Navigate to this item?`,
  ...
  return (
    <div
      className={`min-h-screen ${darkMode ? "bg-gray-900" : "bg-gray-50"} py-6 px-4`}
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
          {/* Library Status */}
          <div
            className={`${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"} rounded-lg shadow border p-4`}
          >
            <div className="flex items-center">
              <div className="text-2xl mr-3">
                {qrLibraries.status.allAvailable
                  ? "✅"
                  : qrLibraries.status.someAvailable
                    ? "⚠️"
                    : "❌"}
              </div>
              <div>
        ...
        {!qrLibraries.status.allAvailable && (
          <div
            className={`mb-6 p-4 rounded-lg ${darkMode ? "bg-yellow-900/20 border-yellow-800" : "bg-yellow-50 border-yellow-200"} border`}
          >
            <div className="flex items-start">
              <div className="text-2xl mr-3">📦</div>
              <div className="flex-1">
                <h3
                  className={`font-medium ${darkMode ? "text-yellow-400" : "text-yellow-800"} mb-2`}
                >
                  Enhance QR Functionality
                </h3>
                <p
                  className={`text-sm ${darkMode ? "text-yellow-300" : "text-yellow-700"} mb-3`}
                >
                  Install QR libraries for full functionality. Current status:{" "}
                  {qrLibraries.status.available}/{qrLibraries.status.total}{" "}
                  libraries available.
                </p>
                <div className="flex items-center space-x-4">
                  <code
                    className={`text-xs ${darkMode ? "bg-gray-800 text-gray-300" : "bg-white text-gray-800"} p-2 rounded font-mono`}
                  >
        ...
        {/* Tab Navigation */}
        <div
          className={`${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"} rounded-xl shadow-lg border overflow-hidden mb-6`}
        >
                  ...
                  >
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                      {scanResults.map((scan) => (
                        <div
                          key={scan.id}
                          className={`p-3 rounded ${darkMode ? "bg-gray-800" : "bg-white"} border ${darkMode ? "border-gray-600" : "border-gray-200"}`}
                        >
                          <div className="flex justify-between items-start">
                            ...
                            {scan.parsed.isOurFormat && scan.parsed.url && (
                              <button
                                onClick={() =>
                                  handleQRNavigation(scan.rawData, navigate)
                                }
                    ...
                    {scanResults.map((scan) => (
                      ...
                      >
                        <div className="flex justify-between items-start mb-3">
                          ...
                          {scan.parsed.isOurFormat && (
                            <span
                              className={`px-2 py-1 rounded-full text-xs ${darkMode ? "bg-green-900/30 text-green-400" : "bg-green-100 text-green-800"}`}
                            >
                              Internal
                            </span>
                          )}
                        ...
                        <div
                          className={`p-2 rounded text-xs ${darkMode ? "bg-gray-800" : "bg-white"} mb-3`}
                        >
                          <pre className="break-all whitespace-pre-wrap max-h-20 overflow-y-auto">
                            {typeof scan.rawData === "object"
                              ? JSON.stringify(scan.rawData, null, 2).substring(
                                  0,
                                  200,
                                ) +
                                (JSON.stringify(scan.rawData).length > 200
                                  ? "..."
                                  : "")
                              : scan.rawData.substring(0, 200) +
                                (scan.rawData.length > 200 ? "..." : "")}
                          </pre>
                        </div>
