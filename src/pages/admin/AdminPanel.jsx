  ...
  return (
    ...
    >
      <div className="flex items-center mb-4">
        <div
          className={`text-3xl mr-4 p-2 rounded ${
    ...
    const fetchRecentActivities = async () => {
      try {
        ...
        usersSnapshot.docs.forEach((doc) => {
          ...
          const updatedAt =
            userData.updatedAt?.toDate() || userData.createdAt?.toDate();
        ...
        activities.map((activity, index) => (
          ...
          >
            <div className="flex items-start space-x-3">
              ...
              <div
                className={`text-xs font-medium ${getSeverityColor(activity.severity)}`}
              >
  ...
  const fetchStats = async () => {
    try {
      ...
      const activeUsers = users.filter(
        (u) => u.status === "active" && u.approved,
      ).length;

      // Get deals data if available
      const deals = dealsSnapshot.docs
        ? dealsSnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }))
        : [];
        ...
        <div className="text-center">
          <div
            className={`inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid ${
              darkMode
                ? "border-blue-500 border-r-transparent"
                : "border-blue-500 border-r-transparent"
            }`}
          ></div>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              >
                Classic administrative interface for{" "}
          ...
          <div className="lg:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {legacyTools.map((tool, index) => (
                <LegacyToolCard key={index} tool={tool} darkMode={darkMode} />
              ))}
            </div>
          </div>
