...
const UserPanel = () => {
  ...
  const filteredUsers = users.filter((userData) => {
    const matchesSearch =
      (userData.displayName?.toLowerCase() || "").includes(
        searchTerm.toLowerCase(),
      ) ||
      (userData.email?.toLowerCase() || "").includes(
        searchTerm.toLowerCase(),
      ) ||
      (userData.businessName?.toLowerCase() || "").includes(
        searchTerm.toLowerCase(),
      );

    const matchesStatus =
      statusFilter === "all" || userData.status === statusFilter;
    const matchesAccountType =
      accountTypeFilter === "all" || userData.accountType === accountTypeFilter;
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div
          className={`text-xl ${darkMode ? "text-cyan-400" : "text-blue-600"}`}
        >
          Loading user panel...
        </div>
      </div>
  ...
  return (
    <div
      className={`container mx-auto px-4 py-8 max-w-7xl ${darkMode ? "text-gray-200" : "text-gray-800"}`}
    >
      {/* Notification */}
      {notification && (
        <div
      ...
      <div className="mb-8">
        <h1
          className={`text-3xl font-bold ${darkMode ? "text-white" : "text-gray-900"} mb-2`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
        <div
          className={`${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"} rounded-xl border p-4`}
        >
        ...
        <div
          className={`${darkMode ? "bg-yellow-900/20 border-yellow-800" : "bg-yellow-50 border-yellow-200"} rounded-xl border p-4`}
        >
          <div className="flex items-center">
            <span className="text-2xl mr-3">⏳</span>
            <div>
              <p
                className={`text-sm ${darkMode ? "text-yellow-400" : "text-yellow-600"} uppercase tracking-wide`}
              >
                Pending Approval
              </p>
              <p
                className={`text-xl font-bold ${darkMode ? "text-yellow-300" : "text-yellow-800"}`}
              >
        ...
        <div
          className={`${darkMode ? "bg-green-900/20 border-green-800" : "bg-green-50 border-green-200"} rounded-xl border p-4`}
        >
          <div className="flex items-center">
            <span className="text-2xl mr-3">✅</span>
            <div>
              <p
                className={`text-sm ${darkMode ? "text-green-400" : "text-green-600"} uppercase tracking-wide`}
              >
                Active Users
              </p>
              <p
                className={`text-xl font-bold ${darkMode ? "text-green-300" : "text-green-800"}`}
              >
                {statusCounts.active}
              </p>
            </div>
        ...
        <div
          className={`${darkMode ? "bg-red-900/20 border-red-800" : "bg-red-50 border-red-200"} rounded-xl border p-4`}
        >
          <div className="flex items-center">
            <span className="text-2xl mr-3">❌</span>
            <div>
              <p
                className={`text-sm ${darkMode ? "text-red-400" : "text-red-600"} uppercase tracking-wide`}
              >
                Rejected
              </p>
              <p
                className={`text-xl font-bold ${darkMode ? "text-red-300" : "text-red-800"}`}
              >
        ...
        <div
          className={`${darkMode ? "bg-orange-900/20 border-orange-800" : "bg-orange-50 border-orange-200"} rounded-xl border p-4`}
        >
          <div className="flex items-center">
            <span className="text-2xl mr-3">⚠️</span>
            <div>
              <p
                className={`text-sm ${darkMode ? "text-orange-400" : "text-orange-600"} uppercase tracking-wide`}
              >
                Suspended
              </p>
              <p
                className={`text-xl font-bold ${darkMode ? "text-orange-300" : "text-orange-800"}`}
              >
      ...
      {statusCounts.pending > 0 && (
        <div
          className={`${darkMode ? "bg-yellow-900/10 border-yellow-800" : "bg-yellow-50 border-yellow-200"} rounded-xl border p-6 mb-8`}
        >
      ...
      {/* Users Table */}
      <div
        className={`${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"} rounded-xl border overflow-hidden`}
      >
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="space-y-2">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-${roleInfo.color}-100 text-${roleInfo.color}-800`}
                        >
                          {roleInfo.label}
                        </span>
                        ...
                        <div className="mt-2">
                          ...
                          >
                            ...
                            <option value="business-seller">
                              Business Seller
                            </option>
                            <option value="business-buyer">
                              Business Buyer
                            </option>
                    ...
                    <td className="px-6 py-4">
                      ...
                      {userData.businessType && (
                        <div
                          className={`text-xs ${darkMode ? "text-blue-400" : "text-blue-600"} capitalize`}
                        >
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        {userData.status === "pending_approval" && (
                          <>
                            <button
                              onClick={() =>
                                handleUserAction(userData.id, "approve")
                              }
                              disabled={processing.has(userData.id)}
                            ...
                            <button
                              onClick={() =>
                                handleUserAction(userData.id, "reject")
                              }
                              disabled={processing.has(userData.id)}
