  ...
  useEffect(() => {
    const fetchSecurityEvents = async () => {
      try {
        ...
        const mediumPriority = events.filter(
          (e) => e.severity === "MEDIUM",
        ).length;
            ...
            >
              <div className="flex-1">
                <div className="flex items-center space-x-2 mb-1">
                  <span
                    className={`text-xs font-medium px-2 py-1 rounded bg-${getSeverityColor(event.severity)}-500 text-white`}
                  >
                    {event.type}
                  </span>
                  <span
    ...
    const fetchUserData = async () => {
      try {
        ...
        const activeUsers = allUsers
          .filter((user) => user.status === "active" && user.approved)
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  >
                    ...
                    {user.businessName && (
                      <div>Business: {user.businessName}</div>
                    )}
                  </div>
                </div>
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-1">
                    <span
                      className={`text-xs font-medium px-2 py-1 rounded bg-${getSeverityColor(log.severity)}-500 text-white`}
                    >
                      {log.type}
                    </span>
                    <span
...
const SystemHealthMonitor = ({ darkMode }) => {
  ...
  const metrics = [
    ...
    { label: "Active Users", value: healthMetrics.activeUsers, type: "normal" },
    {
      ...
      value: healthMetrics.totalProducts,
      type: "normal",
    },
    { label: "Total Orders", value: healthMetrics.totalOrders, type: "normal" },
    {
      label: "Pending Approvals",
      value: healthMetrics.pendingApprovals,
      type: "pending",
    },
    {
      label: "System Uptime",
      value: `${healthMetrics.systemUptime}%`,
      type: "uptime",
    },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="space-y-6">
        ...
        {activeTab === "access" && <AccessControlPanel darkMode={darkMode} />}

        {activeTab === "audit" && <SecurityAuditLog darkMode={darkMode} />}
          ...
          {[
            {
              ...
              action: () =>
                showNotification(
                  "This would suspend all business users",
                  "warning",
                ),
            },
            {
              ...
              color: "blue",
              action: () =>
                showNotification("Security logs would be exported", "success"),
            },
            {
              ...
              action: () =>
                showNotification(
                  "Failed login counters would be reset",
                  "success",
                ),
            },
          ].map((action, index) => (
            ...
            >
              <div
                className={`text-sm font-medium mb-2 text-${action.color}-500`}
              >
                {action.label}
              </div>
              <div
