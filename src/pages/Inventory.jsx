...
const Inventory = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      console.log(
        `📦 Successfully fetched ${fetchedProducts.length} products for ${userAccessLevel}`,
      );
      ...
      const averagePrice =
        totalProducts > 0
          ? fetchedProducts.reduce((sum, p) => sum + p.price, 0) / totalProducts
          : 0;
      ...
      if (err.code === "permission-denied") {
        ...
        console.log(
          "Firestore indexing issue detected, trying simpler query...",
        );
        ...
        if (shouldShowSellerProducts) {
          try {
            ...
            const averagePrice =
              totalProducts > 0
                ? fetchedProducts.reduce((sum, p) => sum + p.price, 0) /
                  totalProducts
                : 0;
          ...
          setError(
            "Database indexing in progress. Please try again in a moment.",
          );
        }
      } else {
  ...
  const handleDeleteProduct = async (productId, productName) => {
    ...
    try {
      ...
      const averagePrice =
        totalProducts > 0
          ? updatedProducts.reduce((sum, p) => sum + p.price, 0) / totalProducts
          : 0;
      ...
      >
        <div className="text-center py-12">
          ...
          <h2 className="text-2xl font-bold mb-2">
            Inventory Not Available for Buyers
          </h2>
          <p className="text-gray-500 mb-4">
            Business buyers can browse and purchase products, but cannot manage
            inventory.
          </p>
  ...
  return (
    ...
    >
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p
                className={`text-2xl font-bold ${stats.lowStockCount > 0 ? "text-red-500" : darkMode ? "text-white" : "text-gray-900"} mt-1`}
              >
                {stats.lowStockCount}
              </p>
            </div>
      ...
      >
        <div
          className={`px-6 py-4 border-b ${darkMode ? "border-gray-700" : "border-gray-100"}`}
        >
          <h2
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        <div
                          className={`w-10 h-10 rounded-lg ${darkMode ? "bg-gray-600" : "bg-gray-200"} flex items-center justify-center mr-3`}
                        >
                          {product.imageUrl ? (
                            <img
