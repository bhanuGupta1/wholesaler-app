...
const Home = () => {
  ...
  const getThemeClasses = () => ({
    ...
    holographicText: darkMode
      ? "holographic-text"
      : "neumorph-holographic-text",
    ...
    ctaCard: darkMode
      ? "cyber-cta-card"
      : "neumorph-cta-card neumorph-holographic",
  });
    ...
    <div className="min-h-screen">
      ...
      <section className="min-h-screen flex items-center justify-center relative z-1">
        ...
        <div className="text-center max-w-6xl mx-auto px-4">
          <div className="mb-12 relative">
            ...
            <h1
              className={`text-6xl md:text-9xl font-bold mb-6 ${theme.title} ${!darkMode ? "neumorph-breathing" : ""}`}
            >
              {user ? (
                <>
                  <span
                ...
                <>
                  ...
                  <br />
                  <span
                    className={`${theme.textSecondary} ${theme.holographicText}`}
                  >
            ...
            <div
              className={`text-2xl md:text-4xl mb-8 ${theme.gradientText} ${!darkMode ? "neumorph-breathing" : ""}`}
            >
              {darkMode
                ? "NEURAL COMMERCE PLATFORM"
                : "TACTICAL BUSINESS COMMAND CENTER"}
            </div>
          </div>

          {/* ENHANCED: Premium description with glass container */}
          <div
            className={`${!darkMode ? "neumorph-glass p-8 rounded-2xl mb-12 neumorph-subtle" : "mb-12"}`}
          >
            <p
              className={`text-xl md:text-2xl max-w-4xl mx-auto leading-relaxed ${theme.description}`}
            >
              {user
                ? darkMode
          ...
          <div
            className={`flex justify-center space-x-12 text-sm ${theme.statusContainer} ${!darkMode ? "p-8 rounded-2xl inline-flex" : ""}`}
          >
            {!darkMode && <div className="neumorph-card-glow"></div>}
            {(darkMode
              ? ["SECURE", "QUANTUM", "NEURAL"]
              : ["SECURE", "TACTICAL", "ARMED"]
            ).map((status, i) => (
              <div key={status} className="flex items-center gap-3">
                <div
                  className={`w-4 h-4 rounded-full ${!darkMode ? "neumorph-status-dot" : ""} ${
                    darkMode
        ...
        {darkMode ? (
          <>
            ...
            <div
              className="absolute bottom-20 right-20 hologram-float opacity-50"
          ...
          <>
            ...
            <div
              className="absolute bottom-20 right-20 neumorph-float opacity-80"
              style={{ animationDelay: "2s" }}
            >
              <div className="neumorph-floating-shape neumorph-floating-sphere neumorph-elevated"></div>
            </div>
            <div
              className="absolute top-1/2 right-10 neumorph-float opacity-70"
              style={{ animationDelay: "4s" }}
            >
              <div className="neumorph-floating-shape neumorph-floating-diamond neumorph-elevated"></div>
            </div>
            <div
              className="absolute top-1/3 left-1/4 neumorph-float opacity-60"
              style={{ animationDelay: "6s" }}
            >
              <div className="neumorph-floating-shape neumorph-floating-hexagon neumorph-elevated"></div>
            </div>
            <div
              className="absolute bottom-1/3 left-20 neumorph-float opacity-70"
              style={{ animationDelay: "8s" }}
            >
              <div className="neumorph-floating-shape neumorph-floating-triangle neumorph-elevated"></div>
            </div>
            <div
              className="absolute top-1/4 right-1/3 neumorph-float opacity-50"
              style={{ animationDelay: "10s" }}
            >
              <div className="neumorph-floating-shape neumorph-floating-cube neumorph-elevated"></div>
            </div>
            <div
              className="absolute bottom-1/4 left-1/3 neumorph-float opacity-65"
      ...
      {user && (
        <section className={theme.section}>
          <div
            className={`container mx-auto px-4 ${theme.premiumContainer} ${!darkMode ? "py-20 rounded-3xl" : ""}`}
          >
            {!darkMode && <div className="neumorph-cta-glow"></div>}

            <h2
              className={`text-5xl font-bold text-center mb-16 ${theme.title}`}
            >
              <span className={`${theme.textPrimary} ${theme.titleEmbossed}`}>
              ...
              {[
                {
                  ...
                  icon: "📋",
                  title: "My Orders",
                  desc: "Advanced command management with tactical tracking protocols",
                },
                {
                  to:
                  ...
                  desc:
                    user.accountType === "business" &&
                    user.businessType === "seller"
                      ? "Strategic product deployment system"
                      : "Launch orders with tactical AI assistance",
                },
                {
                  to: "/products",
                  icon: "🛒",
                  title: "Browse Arsenal",
                  desc: "Explore our tactical equipment ecosystem",
                },
                {
                  to: "/cart",
                  icon: "🛍️",
                  title: "Command Cart",
                  desc: "Strategic cart with intel-based recommendations",
                },
              ].map((item, index) => (
                <Link key={index} to={item.to} className="group">
                  <div className={`${theme.card} neumorph-elevated`}>
                    ...
                    <div className="card-content neumorph-card-content">
                      <div
                        className={`text-5xl mb-6 ${darkMode ? "card-icon" : "neumorph-card-icon"}`}
                      >
                        {item.icon}
                      </div>
                      <h3
                        className={`text-xl font-bold mb-3 ${darkMode ? "card-title" : "neumorph-card-title"}`}
                      >
                        {item.title}
                      </h3>
                      <p
                        className={`${darkMode ? "card-description" : "neumorph-card-description"}`}
                      >
                        {item.desc}
                      </p>
                    </div>
              ...
              {[
                {
                  to: "/products",
                  icon: "🛒",
                  title: "Explore Arsenal",
                  desc: "Discover our tactical-enhanced product catalog with strategic AI intel",
                },
                {
                  to: "/cart",
                  icon: "🛍️",
                  title: "Command Cart",
                  desc: "Experience tactical shopping with predictive battle analytics and precision features",
                },
                {
                  to: "/guest-dashboard",
                  icon: "👁️",
                  title: "Recon Mission",
                  desc: "Preview our platform capabilities with full reconnaissance access and tactical demo features",
                },
              ].map((item, index) => (
                <Link key={index} to={item.to} className="group">
                  <div className={`${theme.card} neumorph-elevated`}>
      ...
      <section className={theme.section}>
        <div className="container mx-auto px-4">
          ...
          <div
            className={`${!darkMode ? "neumorph-glass-strong p-8 rounded-2xl mb-16" : "mb-16"}`}
          >
            <p
              className={`text-xl text-center max-w-4xl mx-auto ${theme.description}`}
            >
              {darkMode
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {[
              ...
              <div key={index} className={`${theme.featureCard} neon-border`}>
                <div
                  className={`inline-flex mb-6 ${feature.color} neumorph-feature-icon`}
                >
                ...
                <h3
                  className={`text-2xl font-bold mb-4 ${feature.color} ${!darkMode ? "neumorph-embossed" : ""}`}
                >
                  {feature.title}
                </h3>
          ...
          <div className="container mx-auto px-4">
            <div className={`max-w-6xl mx-auto ${theme.dashboardCard}`}>
              {darkMode ? (
                ...
                <div className="neumorph-dashboard-glow"></div>
              )}
              <div className="md:flex md:items-center md:justify-between relative z-10">
                <div className="md:flex-1">
                  <div className="flex items-center mb-6">
                    <span
                      className={`text-6xl mr-6 ${darkMode ? "dashboard-icon" : "neumorph-dashboard-icon"}`}
                    >
                      {dashboardInfo.icon}
                    </span>
                    <div>
                      <h2
                        className={`text-4xl font-bold mb-2 ${darkMode ? "text-white" : "text-gray-800 neumorph-embossed"} ${theme.title}`}
                      >
                        Your {dashboardInfo.title}
                      </h2>
                      <p
                        className={`text-xl ${darkMode ? "text-gray-300" : "text-gray-600 neumorph-text-shadow"}`}
                      >
                        Access your personalized command center with advanced
                        tactical features and strategic automation systems
                      </p>
                    </div>
                  </div>
                </div>
                <div className="mt-6 md:mt-0">
                  <Link
                    ...
                    className={`${darkMode ? "cyber-btn cyber-btn-dashboard" : "neumorph-btn neumorph-btn-dashboard neon-border"} text-lg px-8 py-4`}
                  >
                    <span
                      className={darkMode ? "btn-text" : "neumorph-btn-text"}
                    >
        ...
        <div className="container mx-auto px-4 text-center">
          <div className={`max-w-6xl mx-auto ${theme.ctaCard}`}>
            ...
            <div className="relative z-10">
              ...
              <p
                className={`text-xl mb-12 ${theme.description} ${!darkMode ? "neumorph-text-shadow" : ""}`}
              >
                {user
              ...
              <div className="flex flex-wrap gap-6 justify-center">
                {user ? (
                  <>
                    <Link
                      to="/products"
                      className={`${theme.btnPrimary} neon-border`}
                    >
                      <span
                        className={darkMode ? "btn-text" : "neumorph-btn-text"}
                      >
                  ...
                  <>
                    <Link
                      to="/products"
                      className={`${theme.btnSuccess} neon-border`}
                    >
                      <span
                        className={darkMode ? "btn-text" : "neumorph-btn-text"}
                      >
