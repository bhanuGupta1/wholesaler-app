...
const RealDataChart = ({
  ...
  if (!data || data.length === 0) {
    return (
      <div className="mb-6">
        ...
        <p
          className={`text-xs ${darkMode ? "text-gray-400" : "text-gray-500"} mb-3`}
        >
          No data available
        </p>
      </div>
  ...
  return (
    <div className="mb-6">
      <h3
        className={`text-sm font-medium ${darkMode ? "text-gray-300 cyber-glow" : "text-gray-700"} mb-1`}
      >
        {title}
      </h3>
      {description && (
            ...
            return (
              <div key={index} className="flex items-center justify-between">
                <div className="flex items-center">
                  <div
                    className={`w-3 h-3 rounded-full bg-${color}-${500 + index * 100} mr-2 ${darkMode ? "shadow-lg shadow-cyan-500/20" : ""}`}
                  ></div>
                  <span
                ...
                <div className="flex items-center">
                  <span
                    className={`ml-2 text-xs font-medium ${darkMode ? "text-gray-300 cyber-glow" : "text-gray-700"}`}
                  >
        ...
        <div className="space-y-2">
          {data.map((item, index) => (
            <div key={index} className="flex items-center">
              <span
                className={`text-xs w-16 ${darkMode ? "text-gray-400" : "text-gray-500"}`}
              >
                {item.name}
              </span>
              <div className="flex-1 ml-2">
                <div
                  className={`h-4 rounded-full ${darkMode ? "bg-gray-800 border border-cyan-500/30" : "bg-gray-100"} overflow-hidden`}
                >
                  <div
...
const LowStockAlerts = ({ products, darkMode, onBulkRestock }) => {
  ...
  if (lowStockProducts.length === 0) {
    return (
      ...
      >
        <div
          className={`px-6 py-4 border-b ${darkMode ? "border-cyan-500/30" : "border-gray-100"}`}
        >
          <h2
            className={`text-xl font-bold ${darkMode ? "text-white cyber-title cyber-glow" : "text-gray-800 neumorph-title"}`}
          >
  ...
  return (
    ...
    >
      <div
        className={`px-6 py-4 border-b ${darkMode ? "border-cyan-500/30" : "border-gray-100"} flex justify-between items-center`}
      >
        <div>
          <h2
            className={`text-xl font-bold ${darkMode ? "text-white cyber-title cyber-glow" : "text-gray-800 neumorph-title"}`}
      ...
      >
        <label className="flex items-center">
          <input
            type="checkbox"
            checked={
              selectedProducts.size === lowStockProducts.length &&
        ...
        {lowStockProducts.map((product) => (
          <div
            ...
            className={`p-4 border-b ${
              darkMode
                ? "border-cyan-500/20 hover:bg-gray-800/50"
      ...
      >
        <div className="flex space-x-2">
          <button
            onClick={() =>
              setSelectedProducts(
                new Set(criticalStockProducts.map((p) => p.id)),
              )
            }
            className={`text-sm ${
              darkMode
...
const OrderProcessing = ({
  ...
  return (
    ...
    >
      ...
      {/* Status Summary */}
      <div
        className={`p-6 border-b ${darkMode ? "border-cyan-500/20" : "border-gray-200"}`}
      >
        <div className="grid grid-cols-4 gap-4">
          <div className="text-center">
            <div
              className={`text-2xl font-bold ${darkMode ? "text-yellow-400 cyber-glow" : "text-yellow-600"}`}
            >
              {orderCounts.pending}
            </div>
            <div
          ...
          <div className="text-center">
            <div
              className={`text-2xl font-bold ${darkMode ? "text-blue-400 cyber-glow" : "text-blue-600"}`}
            >
          ...
          <div className="text-center">
            <div
              className={`text-2xl font-bold ${darkMode ? "text-green-400 cyber-glow" : "text-green-600"}`}
            >
          ...
          <div className="text-center">
            <div
              className={`text-2xl font-bold ${darkMode ? "text-red-400 cyber-glow" : "text-red-600"}`}
            >
      ...
      <div className="max-h-80 overflow-y-auto">
        {filteredOrders.length > 0 ? (
          <div
            className={`divide-y ${darkMode ? "divide-cyan-500/20" : "divide-gray-200"}`}
          >
  ...
  const generateRealCategoryData = () => {
    ...
    stats.allProducts.forEach((product) => {
      ...
      const estimatedRevenue =
        (product.price || 0) * Math.max(0, 100 - (product.stock || 0)); // Estimate sales
      categoryRevenue[category] =
        (categoryRevenue[category] || 0) + estimatedRevenue;
    });
  ...
  return (
    ...
    >
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          >
            ...
            <div
              className={`text-xs ${darkMode ? "text-gray-500" : "text-gray-400"} mt-1`}
            >
              {stats.totalOrders - stats.pendingOrders} of {stats.totalOrders}{" "}
              orders
            </div>
          </div>
        </div>
        ...
        >
          <div
            className={`text-sm font-medium ${darkMode ? "text-blue-300 cyber-glow" : "text-blue-800"} mb-1`}
          >
            {darkMode ? "🔮 NEURAL INSIGHTS" : "📊 Business Insights"}
          </div>
          <div
            className={`text-xs ${darkMode ? "text-blue-200" : "text-blue-700"} space-y-1`}
          >
            <div>
              • Best performing day:{" "}
              {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {darkMode
            ? "Neural access to core functions"
        ...
        {actions.map((action, index) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`text-xs ${darkMode ? "text-gray-400" : "text-gray-500"} mt-1 text-center`}
              >
                {action.description}
              </span>
            </Component>
...
const ManagerDashboard = () => {
  ...
  const handleUpdateOrderStatus = useCallback(async (orderId, newStatus) => {
    try {
      ...
      showNotification(
        `Order ${orderId.slice(0, 8)} updated to ${newStatus}`,
        "success",
      );
    } catch (error) {
  ...
  const handleBulkRestock = useCallback(async (productIds, amount) => {
    try {
      ...
      showNotification(
        `${productIds.length} products restocked with ${amount} units each`,
        "success",
      );
    } catch (error) {
    ...
    try {
      const reportData = {
        ...
        summary: {
          ...
          completedOrders: stats.allOrders.filter(
        ...
        orderBreakdown: {
          ...
          processing: stats.allOrders.filter((o) => o.status === "processing")
            .length,
          completed: stats.allOrders.filter((o) => o.status === "completed")
            .length,
          cancelled: stats.allOrders.filter((o) => o.status === "cancelled")
            .length,
        },
        inventoryStatus: {
          totalProducts: stats.totalProducts,
          inStock: stats.allProducts.filter((p) => p.stock > 10).length,
          lowStock: stats.allProducts.filter(
  ...
  if (error) {
    return (
      <div
        className={`container mx-auto px-4 py-8 ${darkMode ? "text-white" : "text-gray-800"}`}
      >
  ...
  return (
    <div
      className={`min-h-screen ${darkMode ? "cyberpunk-layout-wrapper" : "neumorph-layout-wrapper"} relative`}
    >
      ...
      <div
        className={`container mx-auto px-4 py-8 max-w-7xl ${darkMode ? "text-gray-200" : "text-gray-800"} relative z-10`}
      >
        {/* Notification */}
        {notification && (
          <div
          ...
          {[
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <p
                    className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-500"} uppercase tracking-wide`}
                  >
                    {stat.title}
                  </p>
                  <p
                    className={`text-2xl font-bold ${
                      darkMode ? "text-white cyber-glow" : "text-gray-900"
                    } mt-1`}
                  >
                    {stat.value}
                  </p>
