...
const CreateOrder = () => {
  ...
  const [processedSelectedProducts, setProcessedSelectedProducts] = useState(
    [],
  ); // NEW: Processed with bulk pricing
  ...
  const calculatePricing = () => {
    ...
    processedSelectedProducts.forEach((product) => {
      ...
      if (product.hasBulkDiscount) {
        itemLevelBulkSavings +=
          (originalPrice - effectivePrice) * product.quantity;
      }
    });
    ...
    if (userRole === "business") {
      ...
      const totalItems = processedSelectedProducts.reduce(
    ...
    return {
      ...
      totalSavings:
        itemLevelBulkSavings + businessDiscount + additionalBulkDiscount, // Grand total savings
      tax,
      total,
    };
  };
  ...
  const getBulkDiscountStatus = () => {
    ...
    const subtotal = processedSelectedProducts.reduce(
      (sum, p) => sum + (p.effectivePrice || p.price) * p.quantity,
      0,
    );
  ...
  const submitOrder = async () => {
    ...
    try {
      ...
      const orderData = {
        ...
        itemCount: processedSelectedProducts.reduce(
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="mb-6">
            ...
            <div className="flex items-center space-x-4">
              ...
              {orderSource === "cart" && (
                <div className="flex items-center space-x-2">
                  <span
                    className={`px-3 py-1 rounded-full text-sm font-medium ${darkMode ? "bg-green-900/30 text-green-400" : "bg-green-100 text-green-800"}`}
                  >
                    🛒 From Cart
                  </span>
                  <button
          ...
          <div className="flex items-center mt-2">
            ...
            >
              {userRole === "business" && (
                <>
                  {userAccountType?.businessType === "seller"
                    ? "🏪"
                    : userAccountType?.businessType === "buyer"
                      ? "🛒"
                      : "🏢"}
                  Business Account{" "}
                  {userAccountType?.businessType
                    ? `(${userAccountType.businessType.charAt(0).toUpperCase() + userAccountType.businessType.slice(1)})`
                    : ""}{" "}
                  - 15% Business + Auto Bulk + 5% Extra Available
                  {(() => {
              ...
              {userRole === "customer" &&
                "👤 Customer Account - 15% Tax Applied + Auto Bulk Pricing"}
              {userRole === "admin" &&
                "⚙️ Admin Account - No Additional Charges"}
          ...
          {pricing.itemLevelBulkSavings > 0 && (
            <div
              className={`mt-4 p-4 rounded-lg ${darkMode ? "bg-green-900/20 border-green-800" : "bg-green-50 border-green-200"} border`}
            >
              <div className="flex items-center">
                ...
                <div>
                  ...
                  >
                    Auto Bulk Pricing Applied! Saving $
                    {pricing.itemLevelBulkSavings.toFixed(2)}
                  </h3>
                  <p
                    className={`text-sm ${darkMode ? "text-green-200" : "text-green-700"}`}
                  >
                    {
                      processedSelectedProducts.filter((p) => p.hasBulkDiscount)
                        .length
                    }{" "}
                    items automatically qualified for volume discounts
                  </p>
                </div>
              </div>
            </div>
        ...
        {error && (
          <div
            className={`mb-6 p-4 rounded-lg border-l-4 ${darkMode ? "bg-red-900/30 border-red-800 text-red-400" : "bg-red-50 border-red-400 text-red-700"}`}
          >
            <div className="flex items-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                {cart.length > 0 && orderSource === "direct" && (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <h3
                          className={`font-medium ${darkMode ? "text-blue-400" : "text-blue-700"}`}
                        >
                          Items in Cart
                        </h3>
                        <p
                        ...
                        >
                          You have {cart.length} item(s) in your cart. Import
                          them to create an order quickly.
                        </p>
                      </div>
                      <button
                        onClick={importFromCart}
                  ...
                  <div className="space-y-3">
                    {filteredProducts.map((product) => (
                      <CompactProductCard
                        ...
                        darkMode={darkMode}
                        isSelected={selectedProducts.find(
                          (p) => p.id === product.id,
                        )}
                      />
                    ))}
                ...
                <div className="mb-8">
                  ...
                  <div className="space-y-3">
                    {processedSelectedProducts.map((product) => {
                      ...
                      const itemSavings = product.hasBulkDiscount
                        ? (product.price - effectivePrice) * product.quantity
                        : 0;

                      return (
                        <div
                          key={product.id}
                          className={`flex justify-between items-center p-4 rounded-lg ${darkMode ? "bg-gray-700" : "bg-gray-50"}`}
                        >
                          <div className="flex items-center space-x-4">
                            <div
                              className={`w-12 h-12 ${darkMode ? "bg-gray-600" : "bg-gray-200"} rounded-lg flex items-center justify-center`}
                            >
                              {product.imageUrl ? (
                                <img
                                ...
                                {product.hasBulkDiscount && (
                                  <div className="flex items-center space-x-1">
                                    <span className="text-xs font-medium text-green-600">
                                      🎯 Auto bulk:{" "}
                                      {product.bulkPricing.bulkDiscount.toFixed(
                                        0,
                                      )}
                                      % off • {product.bulkPricing.bulkTier}+
                                      units
                                    </span>
                                  </div>
                                )}
          ...
          <div className="lg:col-span-1">
            <div
              className={`${darkMode ? "bg-gray-800" : "bg-white"} rounded-xl shadow-lg p-6 sticky top-4`}
            >
                ...
                <div className="space-y-4">
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      {userRole === "business" &&
                        pricing.additionalBulkDiscount > 0 && (
                          <div className="flex justify-between text-green-600 dark:text-green-400">
                            <span>🎯 Extra Bulk Discount (5%):</span>
                            <span>
                              -${pricing.additionalBulkDiscount.toFixed(2)}
                            </span>
                          </div>
                        )}
                      ...
                      <div
                        className={`flex justify-between font-bold text-lg pt-2 border-t ${darkMode ? "border-gray-600" : "border-gray-200"}`}
                      >
                        <span>Total:</span>
              ...
              {/* Security Notice */}
              <div
                className={`mt-4 p-3 rounded-lg ${darkMode ? "bg-blue-900/20 border-blue-800" : "bg-blue-50 border-blue-200"} border text-xs`}
              >
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p
                      className={`${darkMode ? "text-blue-300" : "text-blue-600"} mt-1`}
                    >
                      Your payment information is encrypted and secure.
                    </p>
                    {pricing.totalSavings > 0 && (
                      <p
                        className={`${darkMode ? "text-green-300" : "text-green-600"} mt-1 font-medium`}
                      >
...
const CompactProductCard = ({
  ...
  const getBulkPreview = () => {
    if (!product.bulkPricing || typeof product.bulkPricing !== "object")
      return null;
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4 flex-1">
          <div
            className={`w-12 h-12 ${darkMode ? "bg-gray-600" : "bg-gray-100"} rounded-lg flex items-center justify-center flex-shrink-0`}
          >
            {product.imageUrl ? (
              <img
                src={product.imageUrl}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="space-y-1">
              <div className="flex items-center space-x-4 text-sm">
                {bulkPreview ? (
                  <div className="flex items-center space-x-2">
                    ...
                    <span
                      className={`line-through ${darkMode ? "text-gray-500" : "text-gray-400"}`}
                    >
              ...
              {product.category && (
                <span
                  className={`inline-block px-2 py-1 rounded text-xs ${darkMode ? "bg-gray-700 text-gray-300" : "bg-gray-100 text-gray-600"}`}
                >
                  {product.category}
                </span>
              )}
