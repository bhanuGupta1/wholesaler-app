...
const DealManagement = () => {
  ...
  const handleCleanupExpired = async () => {
    if (!window.confirm("This will deactivate all expired deals. Continue?"))
      return;
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div
          className={`mb-8 p-6 rounded-lg shadow-sm ${darkMode ? "bg-gray-800" : "bg-white"}`}
        >
          ...
          {filteredDeals.map((deal) => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs font-medium whitespace-nowrap ${getStatusColor(deal)}`}
                >
                  {getStatusText(deal)}
                </span>
              </div>
              ...
              <div className="space-y-2 mb-4">
                ...
                {deal.description && (
                  <p
                    className={`text-sm line-clamp-2 ${darkMode ? "text-gray-300" : "text-gray-600"}`}
                  >
                    {deal.description}
                  </p>
                )}
                ...
                >
                  ...
                  {deal.minPurchase > 0 && (
                    <p>Min Purchase: ${deal.minPurchase}</p>
                  )}
                  {deal.maxUses && (
                    <p>
                      Usage: {deal.currentUses || 0} / {deal.maxUses}
                    </p>
                  )}
                </div>
              </div>
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <input
                  type="date"
                  value={
                    editingDeal.expires instanceof Date
                      ? editingDeal.expires.toISOString().split("T")[0]
                      : editingDeal.expires
                  }
                  onChange={(e) =>
                    setEditingDeal({ ...editingDeal, expires: e.target.value })
                  }
                  min={new Date().toISOString().split("T")[0]}
                  className={`w-full px-4 py-2 rounded-lg border ${darkMode ? "bg-gray-700 border-gray-600 text-white" : "bg-white border-gray-300"}`}
                />
        ...
        {showAnalytics && selectedDealAnalytics && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div
              className={`max-w-2xl w-full rounded-lg p-6 max-h-96 overflow-y-auto ${darkMode ? "bg-gray-800" : "bg-white"}`}
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                >
                  ...
                  <p className="text-2xl font-bold">
                    {selectedDealAnalytics.totalUses}
                  </p>
                </div>
                <div
                  className={`p-4 rounded-lg ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}
                >
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    ${selectedDealAnalytics.totalDiscount?.toFixed(2)}
                  </p>
                </div>
                <div
                  className={`p-4 rounded-lg ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}
                >
                  ...
                  <p className="text-2xl font-bold text-blue-600">
                    $
                    {selectedDealAnalytics.totalUses > 0
                      ? (
                          selectedDealAnalytics.totalDiscount /
                          selectedDealAnalytics.totalUses
                        ).toFixed(2)
                      : "0.00"}
                  </p>
              ...
              {selectedDealAnalytics.usageRecords.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {selectedDealAnalytics.usageRecords
                      .slice(0, 10)
                      .map((record, index) => (
                        <div
                          key={index}
                          className={`p-2 rounded text-sm ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}
                        >
                          <p>Order: {record.orderId}</p>
                          <p>Discount: ${record.discountAmount?.toFixed(2)}</p>
                          <p>
                            Date:{" "}
                            {record.usedAt ? formatDate(record.usedAt) : "N/A"}
                          </p>
                        </div>
                      ))}
                  </div>
