...
import { FaQuestionCircle, FaBook, FaVideo, FaDownload, FaSearch, FaTimes, FaChevronDown, FaUser, FaShoppingCart, FaBoxes, FaCog, FaLightbulb, FaPlayCircle, FaFileAlt, FaStar, FaClock } from 'react-icons/fa';
...
const HelpCenter = () => {
  ...
  const faqData = [
    ...
    },
    {
      id: 2,
      category: 'getting-started',
      question: 'What\'s the difference between guest access and creating an account?',
      answer: 'Guest access lets you browse products and view sample orders, but you can\'t place real orders or save data. Creating an account gives you full access to place orders, track order history, manage inventory (if you\'re a seller), and save your preferences. We recommend creating an account for the best experience.',
    ...
    // Ordering & Purchasing
    {
      ...
      answer: 'It depends on the order status: Orders with "Pending" status can usually be modified or cancelled - contact support immediately. Orders marked "Processing" or "Shipped" cannot be changed. Check your order status in the "Orders" section. For urgent changes, use the emergency contact number in the top-right corner.',
    ...
    },
    {
      id: 6,
      category: 'ordering',
      question: 'How do bulk orders work? Can I get discounts for large quantities?',
      answer: 'Yes! Bulk ordering often gets you better prices: 1) Add large quantities to your cart - many products show bulk pricing automatically, 2) For very large orders (500+ items), contact the seller directly for custom pricing, 3) Business accounts often get automatic bulk discounts, 4) Check the product details page for quantity break pricing. Some sellers offer 5-20% discounts for wholesale quantities.',
    ...
    // Account & Profile
    {
      ...
      answer: 'To upgrade to a business account: 1) Go to your Profile (click your avatar in top-right), 2) Click "Account Settings", 3) Select "Upgrade to Business", 4) Provide your business registration details, tax ID, and business address, 5) Wait for admin approval (usually 24-48 hours). Business accounts get access to wholesale pricing, bulk ordering tools, and seller features.',
    ...
    },
    {
      ...
      answer: 'Use the built-in analytics: 1) Go to "Inventory" or your dashboard, 2) Check the "Popular Products" section, 3) Look for view counts and order numbers on each product, 4) Use the date filters to see trends over time, 5) Products with low stock and high demand are your best sellers. Consider restocking these first or raising prices if demand is very high.',
    ...
    // Technical Issues
    {
      id: 11,
      category: 'technical',
      question: 'The website is loading slowly or not working properly. What should I do?',
    ...
    // Payments & Pricing
    {
      ...
      answer: 'We accept multiple payment methods: 1) Credit/Debit cards (Visa, MasterCard, American Express), 2) Bank transfers for large orders, 3) Business accounts can set up NET 30 payment terms, 4) Some sellers accept PayPal, 5) Cash on delivery for local orders (if available in your area). Payment options may vary by seller and order size.',
  ...
  const categories = [
    { id: 'all', name: 'All Topics', icon: FaQuestionCircle, count: faqData.length },
    { id: 'getting-started', name: 'Getting Started', icon: FaUser, count: faqData.filter(f => f.category === 'getting-started').length },
    { id: 'ordering', name: 'Orders & Purchasing', icon: FaShoppingCart, count: faqData.filter(f => f.category === 'ordering').length },
    { id: 'account', name: 'Account & Profile', icon: FaCog, count: faqData.filter(f => f.category === 'account').length },
    { id: 'selling', name: 'Selling & Inventory', icon: FaBoxes, count: faqData.filter(f => f.category === 'selling').length },
    { id: 'technical', name: 'Technical Issues', icon: FaCog, count: faqData.filter(f => f.category === 'technical').length },
    { id: 'payments', name: 'Payments & Pricing', icon: FaUser, count: faqData.filter(f => f.category === 'payments').length },
    { id: 'shipping', name: 'Shipping & Delivery', icon: FaBoxes, count: faqData.filter(f => f.category === 'shipping').length }
  ];
  ...
  const popularTopics = [
    { title: 'How to create your first order', views: 1243, category: 'ordering' },
    { title: 'Understanding bulk pricing', views: 892, category: 'payments' },
    { title: 'Account upgrade process', views: 756, category: 'account' },
    { title: 'Troubleshooting login issues', views: 654, category: 'technical' },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className={`text-xl ${darkMode ? 'text-gray-300' : 'text-gray-600'} max-w-2xl mx-auto`}>
            Get help with orders, account setup, selling products, and everything else you need to succeed
          </p>
        </div>
            ...
            <div className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {quickGuides.map((guide, index) => {
                  ...
                  return (
                    <div key={index} className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow cursor-pointer border-l-4 ${getGuideColor(guide.color)}`}>
                      <div className="flex items-center mb-4">
                        <div className={`w-10 h-10 ${getGuideColor(guide.color)} rounded-lg flex items-center justify-center mr-3`}>
                          <Icon className="text-white text-lg" />
                        </div>
                        <span className={`text-xs px-2 py-1 rounded-full ${getDifficultyColor(guide.difficulty.toLowerCase())}`}>
                          {guide.difficulty}
                        </span>
                      </div>
              ...
              <div className="lg:col-span-2">
                ...
                <div className="space-y-4">
                  {popularTopics.map((topic, index) => (
                    <div key={index} className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow cursor-pointer`}>
                      <div className="flex items-center justify-between">
                        <div>
                          <h3 className={`font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                            {topic.title}
                          </h3>
                          <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'} capitalize`}>
                        ...
                        <div className="text-right">
                          <div className="flex items-center text-gray-500">
                            ...
                            <span className="text-sm">{topic.views.toLocaleString()} views</span>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md p-8`}>
              ...
              {filteredFAQs.length === 0 && (
                <div className="text-center py-12">
                  <FaQuestionCircle className={`text-6xl ${darkMode ? 'text-gray-600' : 'text-gray-300'} mx-auto mb-4`} />
          ...
          <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md p-8`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {quickGuides.map((guide, index) => {
                const Icon = guide.icon;
                return (
                  <div key={index} className={`${darkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg p-6 hover:shadow-lg transition-shadow cursor-pointer`}>
                    <div className="flex items-center mb-4">
                      <div className={`w-12 h-12 ${getGuideColor(guide.color)} rounded-lg flex items-center justify-center mr-4`}>
        ...
        {activeSection === 'videos' && (
          <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md p-8`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[
                { title: 'Getting Started: Your First Order', duration: '3:45', views: '12.5K' },
                { title: 'Setting Up Your Business Account', duration: '5:20', views: '8.2K' },
                { title: 'Managing Your Inventory', duration: '7:15', views: '15.3K' },
                { title: 'Understanding Bulk Pricing', duration: '4:30', views: '6.8K' },
                { title: 'Using QR Codes for Inventory', duration: '6:00', views: '9.1K' },
                { title: 'Troubleshooting Common Issues', duration: '8:45', views: '11.7K' }
              ].map((video, index) => (
                <div key={index} className={`${darkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg overflow-hidden hover:shadow-lg transition-shadow cursor-pointer`}>
