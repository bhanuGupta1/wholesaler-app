...
const AddProduct = () => {
  ...
  const { user } = useAuth();
  const { canManageProducts, userAccessLevel, isAdmin, isManager, isSeller } =
    useAccessControl();
  ...
  useEffect(() => {
    if (formData.stock && !formData.reorderPoint) {
      const autoReorderPoint = Math.max(
        5,
        Math.floor(parseInt(formData.stock) * 0.1),
      );
      setFormData((prev) => ({
        ...prev,
  ...
  const calculateMargins = () => {
    ...
    const profitMargin =
      cost && selling ? (((selling - cost) / selling) * 100).toFixed(1) : 0;
    const markup =
      cost && selling ? (((selling - cost) / cost) * 100).toFixed(1) : 0;
    const discountPercent =
      original && selling
        ? (((original - selling) / original) * 100).toFixed(1)
        : 0;
  ...
  const fetchRecentProducts = async () => {
    ...
    try {
      ...
      if (isAdmin || isManager) {
        ...
        q = query(productsRef, where("createdBy", "==", user.uid), limit(5));
      } else {
        q = query(
          productsRef,
  ...
  const handleDeleteProduct = async (productId, productName) => {
    if (!canDeleteProducts) {
      ...
      if (
        !product ||
        (product.createdBy !== user?.uid && product.ownedBy !== user?.uid)
      ) {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div
            className={`p-3 rounded-lg mb-6 ${darkMode ? "bg-gray-700" : "bg-gray-50"}`}
          >
            <p
              className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}
            >
              Current access level:{" "}
              <span className="font-medium">{userAccessLevel}</span>
            </p>
          </div>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const reorderPoint =
        ...
        Math.max(5, Math.floor(stockQuantity * 0.1));
      ...
      const tags = formData.tags
        ...
        : [
            ...
            formData.name.toLowerCase().split(" ")[0],
          ];
      ...
      const allImages = formData.imageUrl
        ? [formData.imageUrl, ...additionalImages]
        : additionalImages;
  ...
  return (
    <div
      className={`container mx-auto px-4 py-8 max-w-4xl ${darkMode ? "text-gray-200" : "text-gray-800"}`}
    >
      {/* Header */}
      <div className="mb-8">
        <h1
        ...
        >
          Create a comprehensive product listing with pricing, inventory, and
          business details
        </p>
      ...
      {success && (
        <div
          className={`mb-6 p-4 rounded-lg ${darkMode ? "bg-green-900/20 border-green-800 text-green-400" : "bg-green-50 border-green-200 text-green-800"} border`}
        >
      ...
      {error && (
        <div
          className={`mb-6 p-4 rounded-lg ${darkMode ? "bg-red-900/20 border-red-800 text-red-400" : "bg-red-50 border-red-200 text-red-800"} border`}
        >
          <div className="flex items-center">
            <svg
              className="h-5 w-5 mr-2"
      ...
      {/* Form */}
      <div
        className={`${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"} rounded-xl shadow-lg border`}
      >
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                >
                  Cost Price ($)
                  <span
                    className={`text-xs ml-1 ${darkMode ? "text-gray-400" : "text-gray-500"}`}
                  >
                    (Your wholesale cost)
                  </span>
                </label>
            ...
            {(formData.price ||
              ...
              <div
                className={`mt-4 p-4 rounded-lg ${darkMode ? "bg-gray-700/50" : "bg-gray-50"}`}
              >
                <h4
          ...
          <div>
            ...
            >
              🖼️ Product Images
              <span
                className={`text-sm font-normal ml-2 ${darkMode ? "text-gray-400" : "text-gray-500"}`}
              >
                (Add up to 5 images)
              </span>
            </h3>
                      ...
                      >
                        <img
                          ...
                          onError={(e) => {
                            e.target.style.display = "none";
                            e.target.parentNode.innerHTML =
                              '<div class="w-full h-full bg-red-100 flex items-center justify-center"><span class="text-red-500 text-xs">❌</span></div>';
                          }}
                        />
                      </div>
                    )}
              ...
              {(formData.imageUrl || formData.imageUrls.some((url) => url)) && (
                ...
                >
                  ...
                  <div className="flex space-x-2 overflow-x-auto">
                    ...
                    {formData.imageUrl && (
                      <div
                        className={`w-20 h-20 rounded-lg border-2 border-indigo-500 overflow-hidden flex-shrink-0`}
                      >
                        <img
                          src={formData.imageUrl}
                          alt="Primary"
            ...
            <div className="flex items-center my-4">
              ...
              ></div>
              <span
                className={`px-3 text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}
              >
                OR
              </span>
              <div
            ...
            >
              ...
              <p
                className={`text-xs mt-2 ${darkMode ? "text-gray-400" : "text-gray-500"}`}
              >
                💡 Tip: Use the first button to set a primary image, then use
                others to add gallery images
              </p>
            </div>
          </div>
              ...
              {formData.enableBulkPricing && formData.price && (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    ...
                    >
                      📦 10+ units: $
                      {(parseFloat(formData.price) * 0.9).toFixed(2)} (10% off)
                    </div>
                    <div
                      className={`${darkMode ? "text-blue-400" : "text-blue-600"}`}
                    >
                      📦 50+ units: $
                      {(parseFloat(formData.price) * 0.85).toFixed(2)} (15% off)
                    </div>
                    <div
                      className={`${darkMode ? "text-purple-400" : "text-purple-600"}`}
                    >
                      📦 100+ units: $
                      {(parseFloat(formData.price) * 0.8).toFixed(2)} (20% off)
                    </div>
                  </div>
                </div>
              )}
      ...
      >
        <div className="flex items-start">
          ...
          <div>
            ...
            >
              <li>
                • Products automatically owned by you with full management
                rights
              </li>
              <li>
                • Pricing structure tracks cost, selling, and original prices
                for margin analysis
              </li>
              <li>
                • Bulk pricing tiers automatically calculated when enabled
              </li>
      ...
      {canManageProducts && (
        <div
          className={`mt-8 ${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"} rounded-xl shadow-lg border`}
        >
          <div
            className={`px-6 py-4 border-b ${darkMode ? "border-gray-700" : "border-gray-200"} flex justify-between items-center`}
          >
            <div>
              <h3
          ...
          <div className="p-6">
            {loadingProducts ? (
              ...
              <div className="text-center py-8">
                <div
                  className={`text-4xl mb-3 ${darkMode ? "text-gray-600" : "text-gray-400"}`}
                >
                  📦
                </div>
                <p
                  className={`${darkMode ? "text-gray-400" : "text-gray-500"} mb-2`}
                >
                  {isSeller
                ...
                {recentProducts.map((product) => (
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      <div
                        className={`w-12 h-12 rounded-lg ${darkMode ? "bg-gray-600" : "bg-gray-200"} flex items-center justify-center overflow-hidden`}
                      >
                        {product.imageUrl ? (
                          <img
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            {product.originalPrice &&
                              product.originalPrice > product.price && (
                                <span
                                  className={`text-xs line-through ${darkMode ? "text-gray-500" : "text-gray-400"}`}
                                >
